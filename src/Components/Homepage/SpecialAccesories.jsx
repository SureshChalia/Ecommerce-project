     ...
     <div className="flex flex-wrap justify-center mt-7">
      {imgData.map((image, index) => (
        <div key={index} className="w-full sm:w-1/2 md:w-1/3 lg:w-1/4 xl:w-1/4 p-2 relative">
          <Link>
            <img src={image.image} alt={`Image ${index + 1}`} className="w-full h-[20rem] object-cover aspect-square rounded-lg hover:scale-105 transition-all ease-in-out duration-300" />
